...
import { PlayedHand } from '../types';
import { History, Shield, Flame, CheckCircle2, ChevronRight, X } from 'lucide-react';
...
export const GameHistory: React.FC<GameHistoryProps> = ({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-1.5">
          ...
          {onCloseMobile && (
            <button
              ...
              className="sm:hidden p-1 text-slate-400 hover:text-slate-200 rounded-lg bg-slate-800"
              title="Close history"
            >
              <X className="w-4 h-4" />
            </button>
      ...
      {/* History Items List */}
      <div className="flex-1 overflow-y-auto max-h-[260px] sm:max-h-[320px] space-y-2 pr-1 no-scrollbar text-xs">
        {history.length === 0 ? (
